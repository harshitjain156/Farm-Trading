...
// increment decrement cart quantity count
router.post("/increment-count", async (req, res) => {
    const {userId, itemId} = req.body;

    if (!mongoose.Types.ObjectId.isValid(userId) || !mongoose.Types.ObjectId.isValid(itemId)) {
        return res.status(400).send(getErrorResponse("Invalid User ID or Item ID"));
    }
    ...
    try {
        ...
        if (!cart) {
            return res.status(404).send(getErrorResponse("No cart exists for this userId"));
        }

        const itemToUpdate = cart.items.find((item) => item.item.toString() === itemId);

        if (!itemToUpdate) {
            return res.status(404).send(getErrorResponse("Item not found in cart"));
        }
        ...
        return res.send(getSuccessResponse("Item count incremented", cart));
    } catch (error) {
        console.error(error);
...
router.post("/decrement-count", async (req, res) => {
    ...
    try {
        ...
        // Decrement the count of the item, but not below 1
        ...
        return res.send(getSuccessResponse("Item count decremented", cart));
    } catch (error) {
        console.error(error);
        return res.status(500).send(getErrorResponse("Internal server error"));
    }
